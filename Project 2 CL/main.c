...
int yylex(char *yytext, FILE *yyin)
{
    ...
    // check for operators
    if(checkOperator(yytext,yyin))
    {   
        fsetpos(yyin,&position);
        return checkOperator(yytext,yyin);
    }
    ...
    if(checkSingle(yytext,yyin))
    {
        fsetpos(yyin,&position);
        return checkSingle(yytext,yyin);
    }
    //check macro
    if(*yytext=='#')
    {
    ...
    else
    {
        ...
        while(1)
        {
            ...
            *yytext=fgetc(yyin);
            ...
            {
                if(ch==GTEQ_TOK)
                    printf("GTEQ_TOK \tASCII / Value: %d\n",ch);

                if(ch==LTEQ_TOK)
                    printf("LTEQ_TOK \tASCII / Value: %d\n",ch);

                if(ch==NOTEQ_TOK)
                    printf("NOTEQ_TOK \tASCII / Value: %d\n",ch);

                if(ch==PLUSEQ_TOK)
                    printf("PLUSEQ_TOK \tASCII / Value: %d\n",ch);

                if(ch==MINEQ_TOK)
                    printf("MINEQ_TOK \tASCII / Value: %d\n",ch);

                if(ch==DIVEQ_TOK)
                    printf("DIVEQ_TOK \tASCII / Value: %d\n",ch);

                if(ch==MULEQ_TOK)
                    printf("MULEQ_TOK \tASCII / Value: %d\n",ch);
            }
            ...
            else if(ch>=500)
            {
                ...
                if(ch==503)
                {
                    printf("FLOATCONST_TOK \tASCII / Value: %d\n",ch);
                }
